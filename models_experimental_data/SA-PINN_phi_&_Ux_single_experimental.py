...
model_saved_name = "experimental_saved_phi_&_Ux_model_example"
...
# PINN parameters 
NEURONS = 16  # Increased for better capacity
EPOCHS_ADAM = 10000
...
# Trial functions
Ux_trial = lambda y: PINN(torch.cat([y], dim=1))[:,0:1] # * (1 + y) * (1 - y) # torch.Size([y, 1]), normalized 
ϕ_trial = lambda y: ϕ_max * torch.sigmoid(PINN(y)[:,1:2]) * (1 + y) * (1 - y)  # torch.Size([y, 1]), sigmoid keeps bounded between 0 and ϕ_max
...
PINN = NN(neurons=NEURONS, trig_scale=FOURIER_SCALE).to(device)
...
λ_J = nn.Parameter(torch.ones([N_PTS, 1], device=device, requires_grad=True) * λ_INIT)  # for Jstar_divergence, a vector
λ_Σxy = nn.Parameter(torch.ones([N_PTS, 1], device=device, requires_grad=True) * λ_INIT)  # for dΣxystar_dystar, a vector
λ_Σyy = nn.Parameter(torch.ones([N_PTS, 1], device=device, requires_grad=True) * λ_INIT)  # for dΣyystar_dystar, a vector
λ_mass = nn.Parameter(torch.ones([1], device=device, requires_grad=True) * λ_INIT)  # for mass conservation, a scalar
λ_symmetry = nn.Parameter(torch.ones([1], device=device, requires_grad=True) * λ_INIT * 1e2)  # for symmetry, a scalar 
λ_Ux_data = nn.Parameter(torch.ones([y_data.shape[0], 1], device=device, requires_grad=True) * λ_INIT)  # for symmetry, a scalar 
...
def physics_loss():  # physics ensures ∇⋅J = ∇⋅Σ = 0
    ...
    left_wall = 3 * η0 * γ̇ / torch.clamp(4 * torch.pi * ((H/2)*(ystar + 1) + H0)**β, min=MIN_DENOM) * frv
    right_wall = 3 * η0 * γ̇ / torch.clamp(4 * torch.pi * ((H/2)*(1 - ystar) + H0)**β, min=MIN_DENOM) * frv
    scale_L = H / (2 * Ux_max)  # nondimensionalize after calculating it
    L = torch.stack([
...
def ϕ_IC_loss():  # IC ensures mean(ϕ) never changes
    ...
    mass_conservation = torch.tensor([torch.mean(ϕ) - ϕ_average])  # torch.Size([1]), a scalar
...
# Training Loop ---------------------------------------------------------------
# Define parameters
PINN_parameters = PINN.parameters()  # includes ϕ and Ux parameters
λ_parameters = [λ_J, λ_Σxy, λ_Σyy, λ_mass, λ_symmetry, λ_Ux_data]  # self-adaptive weights (include all)

# Define optimizers
PINN_optimizer = torch.optim.Adam([{'params': list(PINN.parameters()), 'lr': LEARNING_RATE_PINN}])
...
for epoch in range(EPOCHS_ADAM):
    ...
    # Forward passes
    ℒ, ℒ_un, ℒ_individuals_un = total_loss()  # forward pass to compute loss
    ...
    λ_scheduler.step(ℒ_un)

    # Update loss history for plotting
    loss_history.append(ℒ_un, ℒ_individuals_un)

    # Visuals
    # print(λ_J)
    print(f"Epoch: {epoch} | Loss: {ℒ_un.item()} Individual Losses: {[f'{l.item():.5f}' for l in ℒ_individuals_un]}")
    if epoch % 50 == 0:
        visualize()

# Save ϕ model when finished
ϕ_saved_path = save_path / model_saved_name
Path(save_path).mkdir(parents=True, exist_ok=True)
torch.save(PINN.state_dict(), ϕ_saved_path)

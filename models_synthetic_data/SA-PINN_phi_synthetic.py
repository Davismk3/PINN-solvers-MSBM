...
# Improvements:
# - Removed wall singularities for ∇⋅J
# - Self-adaptive weight vectors 
# Notes:
# - Loss for Σxy hovers around ~2.5, while the other losses are lower and decreasing
# - η0 in lift force scale_L causes poor convergence
# -----------------------------------------------------------------------------
...
λ_symmetry = nn.Parameter(torch.ones([1], device=device, requires_grad=True) * λ_INIT)  # for symmetry, a scalar 

# β = nn.Parameter(torch.tensor([1.0], device=device, requires_grad=True))
...
# Training Loop ---------------------------------------------------------------
# Define parameters
# PINN_ϕ_parameters = list(PINN_ϕ.parameters()) + [β]  # includes ϕ and Ux parameters
PINN_φ_parameters = [{'params': list(PINN_φ.parameters()), 'lr': LEARNING_RATE_PINN}]
λ_ϕ_parameters = [λ_J, λ_Σxy, λ_Σyy, λ_mass, λ_symmetry]  # self-adaptive weights (include all)

# Define optimizers
PINN_ϕ_optimizer = torch.optim.Adam(PINN_ϕ_parameters, lr=LEARNING_RATE_PINN)
λ_ϕ_optimizer = torch.optim.Adam(λ_ϕ_parameters, lr=LEARNING_RATE_λ)
...
PINN_ϕ_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(PINN_ϕ_optimizer, factor=SCHEDULER_FACTOR, patience=SCHEDULER_PATIENCE, min_lr=SCHEDULER_MIN_LR)
λ_ϕ_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(λ_ϕ_optimizer, factor=SCHEDULER_FACTOR, patience=SCHEDULER_PATIENCE, min_lr=SCHEDULER_MIN_LR)
...
for epoch in range(EPOCHS_ADAM):
    ...
    PINN_ϕ_scheduler.step(ℒ_un)

    # Make gradient negative for λ parameters to achieve ascent, rather than descent
    for λ in λ_ϕ_parameters:
        if λ.grad is not None:
            λ.grad = -λ.grad

    # Gradient ascent & scheduler update
    λ_ϕ_optimizer.step()  # gradient ascent updating self-adaptive weights
    λ_ϕ_scheduler.step(ℒ_un)

    # Update loss history for plotting
    loss_history.append(ℒ_un, ℒ_individuals)

    # Visuals
    print(f"Epoch: {epoch} | Loss: {ℒ_un.item()} | Individual Losses: {[f'{l.item():.5f}' for l in ℒ_individuals]} | ϕ Lr: {PINN_ϕ_scheduler.get_last_lr()} | λ Lr: {λ_ϕ_scheduler.get_last_lr()}")
    if epoch % 50 == 0:
        visualize()
